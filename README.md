...
- **Windowed mode** (`[display] windowed=1`, on by default). Two *fullscreen* instances fight over the exclusive display, so the mod forces Direct3D to create a windowed device. With `reshape=1` (default) each client is then turned into a titled, movable, resizable window — titled `Fable - EgoMP Client <N>` and tiled side by side by number — a few frames into rendering (doing it during init makes Fable's own window/DirectX init "fail" and relaunch, so it's deliberately deferred). Set the window size with `window_width`/`window_height` (0 = auto: half the screen width so two sit side by side). `reshape=0` leaves the game's borderless screen-sized window (alt-tab between clients); `windowed=0` is normal fullscreen for solo play.
