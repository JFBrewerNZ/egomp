...
- **A draggable title bar** (`[display] mouse_unlock`, on whenever `reshape` is on). Fable acquires its DirectInput mouse *exclusively*, which makes Windows ignore clicks on the window frame — the title bar could never start a drag. The mod flips one byte in the game's mouse setup so it acquires in shared (non-exclusive) mode instead; the in-game cursor and camera read the same DirectInput data either way. A small window-proc shim then keeps the Windows arrow visible on the frame but hidden over the game itself (Fable draws its own cursor there). Drag to move, drag edges to resize, and note the game *pauses* while you hold the title bar — that's Windows' modal move loop, normal for windowed games.
